...
아이콘은 브랜치 구조를 대신하지 않는다. 먼저 [branch-archetypes.md](branch-archetypes.md)에서
교과와 수업 목적에 맞는 브랜치 역할을 고른 뒤, 그 의미를 보조하는 Bootstrap Icons 클래스를 붙인다.
예를 들어 과학 `오개념` 브랜치에는 경고성 아이콘보다 학생이 다시 확인할 개념 의미에 맞는
`bi-exclamation-circle`, `bi-lightbulb`, `bi-patch-question` 계열을 상황에 맞게 선택한다.
...
- 수업 목적(`도입`, `정리`, `복습`, `평가`, `탐구`)은 아이콘보다 브랜치 제목·불릿·`pedagogy` 내용으로 드러낼 것
